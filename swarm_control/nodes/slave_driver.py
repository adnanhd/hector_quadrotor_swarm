...
if __name__ == '__main__':
    ...
    while not rospy.is_shutdown():
        ...
        # projection of the current z-axis rotation (yaw) onto x-axix
        cos_swarm_yaw = 0
        # projection of the current z-axis rotation (yaw) onto y-axix
        sin_swarm_yaw = 0
        ...
        for agent in swarm_pose:
            ...
            agent_yaw = Quad2Euler(agent.pose.orientation)[2]
            # projection of the agent's current z-axis rotation (yaw) onto x-axis
            cos_swarm_yaw += math.cos(agent_yaw)
            # projection of the agent's current z-axis rotation (yaw) onto y-axis
            sin_swarm_yaw += math.sin(agent_yaw)

        # hypotenuse of total cumulative yaw angle
        norm_swarm_yaw = math.sqrt(
            cos_swarm_yaw * cos_swarm_yaw + sin_swarm_yaw * sin_swarm_yaw)
        ...
        dot_product = alpha.linear.x * \
            math.cos(slave_yaw) + alpha.linear.y * math.sin(slave_yaw)
        ...
        slave_vel.linear.z = swarm_pose[0].pose.position.z - \
            slave_pose.pose.position.z
        slave_vel.angular.z = ((math.atan2(
            sin_alpha, cos_alpha) - slave_yaw) * K_p)
